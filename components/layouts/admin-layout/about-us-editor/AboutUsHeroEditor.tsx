...
export const AboutUsHeroEditor = ({ data, onChange }: any) => (
  <Stack spacing={4}>
    <Typography variant="h6" sx={{ fontFamily: adelle.style.fontFamily, color: COLORS.PRIMARY_BLUE, fontWeight: 700 }}>Hero Section Settings</Typography>
    ...
    <Box sx={{ mt: 2 }}>
      ...
      />
      <Typography sx={{ fontSize: 12, color: "text.secondary", mt: 1 }}>Note: For large videos, it's better to upload to a service and paste the URL below.</Typography>
    </Box>
    <TextField fullWidth label="Or Video URL" value={data.videoUrl || ""} onChange={(e) => onChange({ ...data, videoUrl: e.target.value })} />
  </Stack>
);
